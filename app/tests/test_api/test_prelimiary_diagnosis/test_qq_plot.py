...
def test_qq_normal(client):
    ts = process_ts()
    result = client.post(
        url='/api/v1/preliminary_diagnosis/data_representations/qq',
        json={
            "timeseries": ts,
            "distribution": "norm"
        }
    ...
    plt.title("Q–Q plot, нормальное распредеелние")
    plt.grid(True, ls="--")
    plt.tight_layout()
...
def test_qq_expon(client):
    ...
    plt.xlabel("Теоретические квантили")
    plt.ylabel("Выборочные квантили")
    plt.title("Q–Q plot, экспон распредеелние")
    plt.grid(True, ls="--")
    plt.tight_layout()
