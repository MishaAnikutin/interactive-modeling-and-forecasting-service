...
class GruParams(BaseModel):
    ...
    encoder_n_layers: int = Field(
        ...
        gt=0,
        le=100,
        title="Количество слоёв GRU",
        description='Количество слоёв GRU'
    )
    encoder_hidden_size: int = Field(
        default=200,
        gt=1,
        le=5000,
        title="Размер скрытого состояния GRU",
        description="Размер скрытого состояния GRU "
...
class GruFitRequest(BaseModel):
    model_data: ModelData = Field(default=ModelData())
    hyperparameters: GruParams = Field(title="Параметры модели GRU")
...
class GruFitResult(ForecastResult):
    pass
